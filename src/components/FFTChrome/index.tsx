...
export function FFTPanel({ children, style, title, cornerStat, inset = 18 }: FFTPanelProps) {
  return (
    <div
      style={{
        ...
        padding: 1,
        borderRadius: 4,
        boxShadow: `0 8px 0 rgba(0,0,0,0.55), 0 0 0 1px ${stFt.panelOuter}, 0 0 28px rgba(0,0,0,0.4)`,
        ...style,
      }}
    >
      <div
        style={{
          background: `linear-gradient(180deg, ${stFt.panelTop} 0%, ${stFt.panelMid} 30%, ${stFt.panelMid} 75%, ${stFt.panelBot} 100%)`,
          border: `1px solid ${stFt.panelBorderMd}`,
          borderRadius: 3,
          boxShadow: `inset 0 1px 0 ${stFt.panelInnerHi}, inset 0 -1px 0 rgba(90,48,24,0.3)`,
          padding: inset,
          position: 'relative',
          color: stFt.panelFg,
        }}
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          height: 9,
          background: stFt.panelBot,
          border: `1px solid ${stFt.panelBorderMd}`,
          borderRadius: 1,
          boxShadow: 'inset 0 1px 0 rgba(90,48,24,0.18)',
        }}
      >
        <div
          style={{
      ...
      >
        {value}
        <span style={{ color: stFt.panelDim, fontWeight: 400 }}>/{max}</span>
      </span>
    </div>
  );
}
...
export function fftButtonStyle(active = false): CSSProperties {
  return {
    fontFamily: stFt.serif,
    fontSize: 13,
    fontWeight: 600,
...
export function UtilityBar({ onTogglePlain, plain }: UtilityBarProps) {
  ...
  return (
    <div
      className="fft-utility-bar"
      style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginBottom: 20, flexWrap: 'wrap' }}
    >
      <a href={resume} style={btnStyle(true)}>
        <span style={{ fontFamily: stFt.pixel, fontSize: 8, color: stFt.bg }}>↓</span>
        Resume
      </a>
      <a href={email} style={btnStyle(false)}>
        Contact
      </a>
